...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_query.setOnClickListener {
            val c: Cursor
            c = if (ed_book.length() < 1)
                dbrw.rawQuery("SELECT * FROM myTable", null)
            else
                dbrw.rawQuery("SELECT * FROM myTable WHERE book LIKE '" + ed_book.text.toString() + "'", null)
            c.moveToFirst()
            items.clear()
        ...
        btn_insert.setOnClickListener {
            if (ed_book.length() < 1 || ed_price.length() < 1) Toast.makeText(this@MainActivity, "欄位請勿留空", Toast.LENGTH_SHORT).show() else {
                try {
                    dbrw.execSQL("INSERT INTO myTable(book,price)VALUES(?,?)", arrayOf<Any>(
                            ed_book.text.toString(),
                            ed_price.text.toString()))
                    Toast.makeText(this@MainActivity, "新增書名" + ed_book.text.toString()
        ...
        btn_update.setOnClickListener {
            if (ed_book.length() < 1 || ed_price.length() < 1) Toast.makeText(this@MainActivity, "欄位請物留空", Toast.LENGTH_SHORT).show() else {
                try {
                    dbrw.execSQL("UPDATE myTable SET price = " + ed_price.text.toString() + " WHERE book LIKE '" + ed_book.text.toString() + "'")
                    Toast.makeText(this@MainActivity, "更新書名" + ed_book.text.toString()
                            + "      價格" + ed_price.text.toString(), Toast.LENGTH_SHORT).show()
                    ed_book.setText("")
                    ed_price.setText("")
        ...
        btn_delete.setOnClickListener {
            if (ed_book.length() < 1) Toast.makeText(this@MainActivity, "書名請勿留空", Toast.LENGTH_SHORT).show() else {
                try {
                    ...
                    Toast.makeText(this@MainActivity, "刪除書名" + ed_book.text.toString(),
